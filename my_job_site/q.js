try {
  window.onload = function () {
    var timingVars = ["connectEnd", "connectStart", "domComplete", "domContentLoadedEventEnd",
    ...
    if (window.performance && window.performance.timing) {
      ...
      timingVars.forEach(function (timingVar) {
        if (timingVar in perfData) {
          params += '&' + timingVar + '=' + perfData[timingVar];
        }
      });

      (new Image).src = '//test.i-exam.ru/webstat?domain=' + window.location.host +
        '&protocol=' + window.location.protocol + params;
    }
  }
} catch (e) {
}
